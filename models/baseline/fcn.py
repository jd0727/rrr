from .vgg import VGGBkbn, VGGMain
...
class FCN32VGGMain(VGGBkbn, ImageONNXExportable):
    ...
    def __init__(self, nums_repeat=(1, 1, 2, 2, 2), act=ACT.RELU, norm=NORM.BATCH, num_cls=20, head_channel=4096,
                 img_size=(224, 224), in_channels=3):
        super(FCN32VGGMain, self).__init__(nums_repeat=nums_repeat, act=act, norm=norm, in_channels=in_channels)
...
class FCN16VGGMain(VGGBkbn, ImageONNXExportable):
    def __init__(self, nums_repeat=(1, 1, 2, 2, 2), act=ACT.RELU, norm=NORM.BATCH, num_cls=20, head_channel=4096,
                 img_size=(224, 224), in_channels=3):
        super(FCN16VGGMain, self).__init__(nums_repeat=nums_repeat, act=act, norm=norm, in_channels=in_channels)
        self.stage6 = FlatHead(in_channels=512, head_channel=head_channel, out_channels=num_cls + 1, act=act, norm=norm)
...
class FCN8VGGMain(VGGBkbn, ImageONNXExportable):
    def __init__(self, nums_repeat=(1, 1, 2, 2, 2), act=ACT.RELU, norm=NORM.BATCH, num_cls=20, head_channel=4096,
                 img_size=(224, 224), in_channels=3):
        super(FCN8VGGMain, self).__init__(nums_repeat=nums_repeat, act=act, norm=norm, in_channels=in_channels)
        self.stage6 = FlatHead(in_channels=512, head_channel=head_channel, out_channels=num_cls + 1, act=act, norm=norm)
        self.upsamper2 = CTpa(in_channels=num_cls + 1, out_channels=num_cls + 1, kernel_size=4, stride=2)
...
class FCN(OneStageSegmentor):
    ...
    @staticmethod
    def V32A(device=None,  num_cls=20, img_size=(224, 224), in_channels=3):
        backbone = FCN32VGGMain.A(act=ACT.RELU, norm=NORM.BATCH, num_cls=num_cls, img_size=img_size,
                                  in_channels=in_channels)
        return FCN(backbone=backbone, device=device,  num_cls=num_cls, img_size=img_size)

    @staticmethod
    def V16A(device=None,  num_cls=20, img_size=(224, 224), in_channels=3):
        backbone = FCN16VGGMain.A(act=ACT.RELU, norm=NORM.BATCH, num_cls=num_cls, img_size=img_size,
                                  in_channels=in_channels)
        return FCN(backbone=backbone, device=device,  num_cls=num_cls, img_size=img_size)

    @staticmethod
    def V8A(device=None,  num_cls=20, img_size=(224, 224), in_channels=3):
